...
class DropoutBalanceLoss(nn.Module):
    ...
    def __init__(self, config: GlobalConfig):
        ...
        # self.easy_attr_lowest_l   r = config.easy_attr_lowest_lr
        ...
        self.loss = torch.zeros(1, config.dataset_config.attribute_num).cuda()
        ...
        self.weight_num = torch.zeros(config.dataset_config.attribute_num)
    ...
    def forward(self, pred, target, *args, **kwargs):
        ...
        self.config.drop_rate_record += dropout_rate.cpu().detach().numpy()
        # print(dropout_rate > 0.5)
        # dropout method
        idxs = (g >= edges[self.bins - self.dropout_scope]) & (g < edges[self.bins])
        self.weight_num += idxs.sum(0).cpu().float()
        drop_idxs = rand_mat.gt(dropout_rate).float()

        weights = weights - drop_idxs * idxs.float()
        self.config.drop_num_record += batch_size - weights.sum(0).cpu().numpy()

        # bins
        # num_in_bin = torch.zeros((class_size, self.bins))
    ...
    #     a = self.weight_num
    #     b = self.loss
    #     self.weight_num = torch.zeros(self.config.dataset_config.attribute_num)
    #     self.loss = torch.zeros(1, self.config.dataset_config.attribute_num).cuda()
    #     return a, b.cpu()
